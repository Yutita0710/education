      ...
      <div class="p-6 overflow-y-auto max-h-[90vh]">
        <form @submit.prevent="updateCurriculum" class="space-y-4">
          ...
          <div class="mb-4 relative">
            <label class="block font-bold mb-1">ชื่อสถาบัน <span class="text-red-500">*</span></label>
            <v-select
              class="font-[15px]"
          ...
          <div class="grid grid-cols-2 gap-4 items-end w-full">
            ...
            <div>
              ...
              <Listbox v-model="selectedCurriculumType" as="div" class="relative w-full rounded-lg border px-2 py-[0.15rem]">
                <ListboxButton class="relative w-full inline-flex items-center justify-between px-3 py-2">
                  <span class="truncate">{{ selectedCurriculumType?.name || "หลักสูตร" }}</span>
                  <ChevronUpDownIcon class="w-5 h-5 text-gray-500" aria-hidden="true" />
                </ListboxButton>
                <ListboxOptions class="absolute z-20 mt-2 max-h-60 w-full md:w-[14rem] overflow-auto rounded-xl bg-white py-1 text-sm shadow-lg ring-1 ring-black/5 left-0">
                  <ListboxOption
            ...
            <div class="flex flex-row items-center gap-2 w-full">
              ...
              <v-select
                ...
                :disabled="isLoading || !(curriculumYearOptions && curriculumYearOptions.length)"
          ...
          <div class="mb-4 relative">
            ...
            <Listbox v-model="selectedDegree" as="div" class="relative w-full rounded-lg border px-2 py-[0.15rem]">
              <ListboxButton class="relative w-full inline-flex items-center justify-between px-3 py-2">
                <span class="truncate">{{ selectedDegree?.name || "ระดับการศึกษา" }}</span>
                <ChevronUpDownIcon class="w-5 h-5 text-gray-500" aria-hidden="true" />
              </ListboxButton>
              <ListboxOptions class="absolute z-20 mt-2 max-h-60 w-full md:w-[14rem] overflow-auto rounded-xl bg-white py-1 text-sm shadow-lg ring-1 ring-black/5 left-0">
                <ListboxOption
                ...
                >
                  ...
                  <span v-if="selectedDegree?.id === degree.id" class="absolute inset-y-0 left-3 flex items-center text-blue-600">
                    <CheckIcon class="w-5 h-5" aria-hidden="true" />
                  </span>
                </ListboxOption>
              </ListboxOptions>
          ...
          <div>
            <div class="flex flex-row gap-2 items-center">
              ...
              <div class="mb-2">
                <p v-if="!selectedTypes.length" class="mt-1 text-xs text-red-600">เลือกได้มากกว่า 1 ประเภท</p>
              </div>
            </div>
            <ul v-if="typeOptions.length" class="flex flex-row gap-1 w-full">
              <li v-for="opt in typeOptions" :key="opt.id" class="w-full">
                <label :for="`type-${opt.id}`" class="flex items-center gap-1 px-2 py-1 cursor-pointer hover:bg-gray-50 rounded">
                  <input
                    :id="`type-${opt.id}`"
                    type="checkbox"
                    :value="Number(opt.id)"
                  ...
                  <span class="text-sm">
                    {{ opt.name }}
                    <span v-if="opt.name === ALL_LABEL" class="text-xs text-gray-500">(เลือกทั้งหมด)</span>
                  </span>
                </label>
              </li>
            </ul>
          ...
          <div>
            ...
            <textarea v-model="form.remark" rows="2" class="w-full border px-4 py-2 rounded-xl resize-none"></textarea>
          </div>

          <!-- มติ/วันที่ -->
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
            <div>
              <label class="block font-bold mb-1">มติการประชุมครั้งที่ <span class="text-red-500">*</span></label>
              <input v-model="form.meeting_no" type="text" class="w-full border px-4 py-2 rounded-xl" placeholder="เช่น 1/2568" />
            </div>
            <div>
              <label class="block font-bold mb-1">วันที่ประชุม <span class="text-red-500">*</span></label>
              <VueDatePicker
                v-model="meetingDate"
                :format="formatToThai"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
            <div>
              <label class="block font-bold mb-1">ปีที่เริ่มต้น (พ.ศ.) <span class="text-red-500">*</span></label>
              <v-select
                class="font-[15px] flex-1 min-w-0"
                ...
                label="name"
                :reduce="(opt) => opt.value"
                :disabled="isLoading || !(startYearOptions && startYearOptions.length)"
                placeholder="กรุณาเลือกปี พ.ศ."
                :input-id="'start-year'"
              />
            </div>
            ...
            <div>
              <label class="block font-bold mb-1">ปีที่สิ้นสุด (พ.ศ.) <span class="text-red-500">*</span></label>
              <input
                id="end-year"
                ...
                @input="selectedEndYear = String($event.target.value).replace(/[^0-9]/g, '').slice(0, 4)"
              />
            </div>

            <div>
              <label for="ispublic" class="block mb-2 font-medium text-gray-900">สถานะการเผยแพร่</label>
              <Listbox v-model="selectedIspublic" as="div" class="relative w-full rounded-lg border px-2 py-[0.15rem]">
                <ListboxButton class="relative w-full inline-flex items-center justify-between px-3 py-2">
                  <span class="truncate">{{ selectedIspublic?.name || "สถานะการเผยแพร่" }}</span>
                  <ChevronUpDownIcon class="w-5 h-5 text-gray-500" aria-hidden="true" />
                </ListboxButton>
                <ListboxOptions class="absolute z-20 mt-2 max-h-60 w-full md:w-[14rem] overflow-auto rounded-xl bg-white py-1 text-sm shadow-lg ring-1 ring-black/5 left-0">
                  <ListboxOption
                  ...
                  >
                    <span class="block truncate">{{ opt.name }}</span>
                    <span v-if="selectedIspublic?.id === opt.id" class="absolute inset-y-0 left-3 flex items-center text-blue-600">
                      <CheckIcon class="w-5 h-5" aria-hidden="true" />
                    </span>
                  </ListboxOption>
                </ListboxOptions>
            ...
            <div>
              <label for="status" class="block mb-2 font-medium text-gray-900">สถานะการใช้งาน</label>
              <Listbox v-model="selectedStatus" as="div" class="relative w-full rounded-lg border px-2 py-[0.15rem]">
                <ListboxButton class="relative w-full inline-flex items-center justify-between px-3 py-2">
                  <span class="truncate">{{ selectedStatus?.name || "สถานะการใช้งาน" }}</span>
                  <ChevronUpDownIcon class="w-5 h-5 text-gray-500" aria-hidden="true" />
                </ListboxButton>
                <ListboxOptions class="absolute z-20 mt-2 max-h-60 w-full md:w-[14rem] overflow-auto rounded-xl bg-white py-1 text-sm shadow-lg ring-1 ring-black/5 left-0">
                  <ListboxOption
                  ...
                  >
                    ...
                    <span v-if="selectedStatus?.id === s.id" class="absolute inset-y-0 left-3 flex items-center text-blue-600">
                      <CheckIcon class="w-5 h-5" aria-hidden="true" />
                    </span>
                  </ListboxOption>
                </ListboxOptions>
 ...
 * =======================================================*/
const formatToThai = (date) => (date ? dayjs(date).add(543, "year").format("DD/MM/YYYY") : "");
...
function ensureYearOption(year) {
  ...
  if (!exists) {
    ...
    curriculumYearOptions.value = [opt, ...curriculumYearOptions.value].sort((a, b) => b.value - a.value);
    startYearOptions.value = [opt, ...startYearOptions.value].sort((a, b) => b.value - a.value);
    endYearOptions.value = [opt, ...endYearOptions.value].sort((a, b) => b.value - a.value);
  }
}
...
function ensureCollegeOption(id, fallback) {
  ...
  if (!exists && fallback?.name) {
    const label = [fallback.name, (fallback.campus ?? "").trim()].filter(Boolean).join("  ");
...
function ensureDegreeOption(id, fallback) {
  ...
  if (!exists && fallback?.name) filteredDegrees.value.unshift({ id: nid, ...fallback });
}

function ensureCurriculumNameOption(name) {
  if (!name || typeof name !== "string") return;
  const exists = curriculumNameOptions.value.some((o) => o?.name?.trim() === name.trim());
  if (!exists) curriculumNameOptions.value.unshift({ name: name.trim() });
}

function normalizeTypeField(typeField) {
  if (Array.isArray(typeField)) return typeField.map(Number).filter(Number.isFinite);
  if (typeof typeField === "string")
    return typeField.split(/[,\s]+/).map((s) => parseInt(s, 10)).filter(Number.isFinite);
  return [];
}
...
async function fetchStartYears() {
  ...
  const apiYears = Array.isArray(res?.data?.startYears) ? res.data.startYears : [];
  const nums = [...new Set(apiYears.map((y) => Number(y)))].filter((y) => Number.isFinite(y) && y >= 2400 && y <= 2699);

  const currentBE = dayjs().year() + 543;
  const minBE = nums.length ? Math.min(...nums) : 2400;

  const range = Array.from({ length: currentBE - minBE + 1 }, (_, i) => currentBE - i)
...
function syncEndYearOptionsByStart(startVal) {
  ...
  endYearOptions.value = startYearOptions.value.filter((o) => o.value >= startVal);
  if (Number.isFinite(selectedEndYear.value) && selectedEndYear.value < startVal) {
...
async function fetchCurriculumNames() {
  ...
  curriculumNameOptions.value = names.filter(Boolean).map((n) => ({ name: String(n).trim() }));
}
...
async function fetchColleges() {
  try {
    const params = { sort: "institute_group", order: "ASC", search: search.value.trim(), active: 1 };
...
async function prefetchMasters() {
  ...
  await fetchTypes();
  await Promise.all([fetchDegrees(), fetchColleges(), fetchStartYears(), fetchCurriculumNames()]);
...
watch(selectedEndYear, (y) => {
  ...
  if (/^\d{4}$/.test(str)) form.end_year = str;
  else form.end_year = "";
});
...
watch(selectedTypes, (v) => {
  const normalized = Array.from(new Set((v || []).map(Number).filter(Number.isFinite)));
...
function applyIncomingTypes(incoming) {
  if (!realTypeIds.value.length) {
    pendingIncomingTypes.value = Array.isArray(incoming) ? [...incoming] : incoming;
    return;
  }
  ...
  if (allId != null && arr.includes(allId)) {
    ...
    const hasAllReal = allReal.length > 0 && allReal.every((id) => arr.includes(id));
    selectedTypes.value = hasAllReal && allId != null ? [allId, ...allReal] : arr;
  }
}
...
function setTypesFromRecord(typeFieldFromRecord) {
  ...
  const allSelected = chosen.length > 0 && chosen.length === realTypeIds.value.length;
...
function populateFromCurriculum(c) {
  ...
  selectedCollege.value = Number.isFinite(Number(collegeId)) ? Number(collegeId) : null;

  const degreeId = c.degree_id ?? c.degree?.id ?? null;
  ensureDegreeOption(degreeId, c.degree);
  selectedDegree.value = (filteredDegrees.value || []).find((d) => Number(d.id) === Number(degreeId)) ?? null;

  ensureCurriculumNameOption(c.name);
  selectedCurriculum.value = c.name || null;

  // สถานะต่าง ๆ
  selectedIspublic.value = ispublicOptions.find((x) => x.value === Boolean(c.is_curriculum_published)) ?? ispublicOptions[0];
  selectedStatus.value = statusOptions.find((x) => x.id === Number(c.active)) ?? statusOptions[0];

  // Description → แยกชนิด/ปี
  const { typeName, yearBE } = parseTypeYearFromDescription(c.description || "");
...
async function updateCurriculum() {
  ...
  const noYear = selectedCurriculumYear.value === null || selectedCurriculumYear.value === undefined;
  if (noYear) {
  ...
  if (!selectedTypes.value.length)
    return Swal.fire({ icon: "warning", title: "กรุณาเลือก 'หลักสูตรสำหรับ' อย่างน้อย 1 รายการ" });
  ...
  if (dayjs(meetingDate.value).isAfter(dayjs(), "day")) {
    await Swal.fire({ icon: "warning", title: "วันที่ประชุมต้องไม่เกินวันที่ปัจจุบัน" });
    return;
  }

  if (!form.start_year)
    return Swal.fire({ icon: "warning", title: "กรุณาเลือกปีที่เริ่มต้น (พ.ศ.)" });

  const start = parseInt(form.start_year);
  const end = parseInt(form.end_year);
  if (Number.isFinite(start) && Number.isFinite(end) && end < start)
    return Swal.fire({ icon: "warning", title: "ปีที่สิ้นสุดต้องไม่น้อยกว่าปีที่เริ่มต้น" });
  ...
  try {
    ...
    if (status === 400) {
      await Swal.fire({ icon: "warning", title: "บันทึกไม่สำเร็จ", text: messageText || "กรุณาตรวจสอบข้อมูล" });
    } else {
      await Swal.fire({ icon: "error", title: "เกิดข้อผิดพลาด", text: messageText || "ไม่สามารถบันทึกข้อมูลได้" });
    }
  } finally {
